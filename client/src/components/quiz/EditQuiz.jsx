...
class EditQuiz extends Component {
  ...
  componentDidMount() {
    this.callGetQuiz()
      .then((res) => {
        this.setState({ name: res.quizName });
        this.setState({ time: res.quizTimeLimit });
        this.setState({ questions: eval('(' + res.quizQuestions + ')') });
      })
      .catch((err) => this.setState({ error: err }));
  }
  callGetQuiz = async () => {
    const response = await fetch(
      `/api/employers/${this.state.id}/quizzes/${this.state.quizID}`,
      {
        method: 'GET',
